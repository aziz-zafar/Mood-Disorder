...
for (outcome in outcomes){
  ...
  data_l = data.frame(lapply(data,as.logical))
  rules_all <- apriori(data_l, parameter = list(supp=0.03, conf=0.7,maxlen=10,target ="rules"),appearance = list(default="lhs",rhs=outcome), control=list(verbose = FALSE, load = T, memopt = F))
    rules_all = arules::sort(rules_all, decreasing = TRUE, na.last = NA, 
      by = c("confidence", "lift", "support"), order = FALSE)
    f_out =paste("arules_", outcome, "_all.csv", sep ="")
    arules::write(rules_all, file = f_out, sep = ",")
  ...
  for (sex in c("Male", "Female")){
    ...
    rhs_rule = outcome
    rules <- apriori(data_set, parameter = list(supp=0.03, conf=0.5,maxlen=10,target ="rules"),appearance = list(default="lhs",rhs=rhs_rule), control=list(verbose = FALSE, load = T, memopt = F))
   ...
   filter(!row_number() %in% which(to.remove))
    ...
    if ("SOCIOSTATUS_3.0" %in% matrix_names){
      matrix_names = str_replace(matrix_names, "SOCIOSTATUS_3.0", "UMC")}
    if ("SOCIOSTATUS_4.0" %in% matrix_names){
      matrix_names = str_replace(matrix_names, "SOCIOSTATUS_4.0", "UC")}
    ...
    tiff(filename = paste("graph_", sex, ".tiff", sep =""), width =10, height =8,
         units = "in", res =300)
    coords = layout_(M_graph, as_star())
    if (sex == "Female"){coords = coords[match(F_graph_names, M_graph_names),]}
    #coords = layout_(graph, as_star())
    plot(graph, layout = na.omit(coords),
         edge.width = exp(proportions(E(graph)$weight)*10), 
         normalize = F,
         ...
         vertex.label.dist = case_when(
           coords[,1] <0 ~-2.7,
           coords[,2] == 0 & coords[,1]>0 ~ 4.3,
           coords[,1] >0 ~ 2.7,
           TRUE ~ ifelse(sex == "Female", 3.1,2.3)),
...
create_bargraph <- function(gene1, gene2, SNP1,SNP2, Mood) {
  ...
  #View(data_subset) Use this code to remove a specific outlier (Male, PER3BCRY1AACG)
  ...
  anova_formula = as.formula(paste(Mood, "~ Gender*Genotype", sep = ""))
  subset_anova = aov(anova_formula, data = data_subset)
  ...
  print(leveneTest(anova_formula, data = data_subset))
  ...
  comp_means =multcomp::cld(TukeyInt, Letters = LETTERS)
  print(comp_means)
  sink()
  ...
  summary_gendered = summarize(group_by(data_subset, Genotype, Gender),
                               Average = mean(!!sym(Mood)) -8, SE = (sd(!!sym(Mood))/sqrt(length(!!sym(Mood)))))
  comp_means$emmean = comp_means$emmean - 8 
  plot_2 = ggplot(comp_means, aes(Gender, emmean, fill = Genotype)) +
    geom_bar(stat = "identity", color = 'black',
                                          width=0.85, position=position_dodge(0.95))+
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean +SE), width=0.2, position = position_dodge(0.95)) +labs(y=paste("PROMIS"," Score",sep=""), x = "Gender", fill = gene_combination) + theme_classic() + scale_y_continuous(expand = c(0,0), limits = c(0, 25),breaks=c(0,7,12, 17,22), labels=c(8, 15,20, 25,30))+
    scale_fill_manual(values=c("#FFFFFF", "#808080"))+
    theme(
      axis.text=element_text(size=32), axis.title = element_text(size = 32),
      legend.title = element_text(size= 24), legend.text = element_text(size =24),
      legend.position = c(.85, .9),
      panel.border = element_rect(colour = "black", fill=NA),
      legend.background = element_blank(),
      legend.box.background = element_rect(colour = "black"))+
    geom_text(aes(label = gsub(" ", "", .group), y = emmean +SE), vjust = -0.5,
  ...
  summary_gendered_gene = summarize(group_by(data_subset, !!sym(gene1),!!sym(gene2), Gender), Average = mean(!!sym(Mood)) - 8, SE = (sd(!!sym(Mood))/sqrt(length(!!sym(Mood)))))
  ...
  geom_errorbar(aes(ymin = Average - SE, ymax = Average +SE), width=0.2, position = position_dodge(width = 0.9, preserve = 'single')) +labs(y=paste("PROMIS"," Score",sep=""), x = gene1) + theme_classic() + 
    scale_y_continuous(expand = c(0,0), limits = c(0,23),breaks=c(0,7,12, 17, 22), labels=c(8, 15,20, 25,30))+
...
``` {r Additional Bar Graphs, warning = F}
library(ggplot2)
library(tiff)
Average = 20.68
misalignment_data = data.frame(Actual = c(21.63158,18.68,23,19), SE = c(0.977143797,1.247298619,1.446666667,1.503997451),
                               Type = c("Delayed","Delayed", "Advanced", "Advanced"),
                               Chronotype = c("ET", "MT", "ET", "MT"))
misalignment_plot = ggplot(misalignment_data, aes(x = Chronotype, Actual - 8,
                                                  fill = Type)) +
    geom_bar(stat = "identity", color = 'black',
                                          width=0.85, position=position_dodge(0.95))+
  labs(y=paste("PROMIS"," Score",sep=""), x = "Chronotype") +
  geom_errorbar(aes(ymin = Actual - SE -8, ymax = Actual + SE-8), position = position_dodge(0.95))+
  theme_classic() + scale_y_continuous(expand = c(0,0), limits = c(0, 20),breaks=c(0,7,12, 17), labels=c(8, 15,20, 25))+
    scale_fill_manual(values=c("#FFFFFF", "#808080"))+
    theme(
      axis.text=element_text(size=32), axis.title = element_text(size = 32),
      legend.title = element_text(size= 24), legend.text = element_text(size =24),
      ...
      legend.background = element_blank(),
      legend.box.background = element_rect(colour = "black"))+
  geom_hline(yintercept = Average -8, color = "red", linetype = "dashed")

tiff(filename = "misalignment_graph.tiff",
       width =7.5, height =7.7,
         units = "in", res =300)  
  misalignment_plot
  dev.off()
  
  
misalignment_plot2 = ggplot(misalignment_data, aes(x = Type, Actual - 8,
                                                  fill = Chronotype)) +
    geom_bar(stat = "identity", color = 'black',
                                          width=0.85, position=position_dodge(0.95))+
  labs(y=paste("PROMIS"," Score",sep=""), x = "Type") +
  theme_classic() + scale_y_continuous(expand = c(0,0), limits = c(0, 20),breaks=c(0,7,12, 17), labels=c(8, 15,20, 25))+
    scale_fill_manual(values=c("#FFFFFF", "#808080"))+
  geom_errorbar(aes(ymin = Actual - SE -8, ymax = Actual + SE-8), position = position_dodge(0.95),
                width = 0.2)+
...
tiff(filename = "misalignment_graph2.tiff",
       width =7.5, height =7.7,
         units = "in", res =300)  
  misalignment_plot2
  dev.off()
