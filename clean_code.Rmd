...
for (outcome in outcomes){
  ...
  for (sex in c("Female", "Male")){
   ...
   filter(!row_number() %in% which(to.remove))
    ...
    tiff(filename = paste("graph_", sex, ".tiff", sep =""), width =8, height =8,
         units = "in", res =300)
    coords = layout_(graph, as_star())
    
    plot(graph, layout = coords,
         edge.width = log(exp(proportions(E(graph)$weight)*100)/5), 
         normalize = T,
         edge.arrow.mode = 0, vertex.color = new_sz, vertex.size =proportions(lift_total$Count)*50,
         vertex.label.cex = 0.7,vertex.label.font = 2, vertex.label.color= "black",
    ...
    legend("bottomright",
           ...
           col = "grey",
           lwd = c(quantile(log(exp(proportions(E(graph)$weight)*100)/5),
                            c(0.75,0.5,0.25))),
           cex = 0.8,
           title = "Co-occurence")    
    dev.off()
...
create_bargraph <- function(gene1, gene2, SNP1,SNP2, Mood) {
  ...
  geom_errorbar(aes(ymin = Average - SE, ymax = Average +SE), width=0.2) +
    ...
    scale_fill_manual(values=c("#FFFFFF", "#808080"))+geom_hline(yintercept =20)+coord_cartesian(ylim= c(20,60))
  
  
  summary_gendered = summarize(group_by(data_subset, Genotype, Gender), Average = mean(!!sym(Mood)), SE = (sd(!!sym(Mood))/sqrt(length(!!sym(Mood)))))
  ...
  geom_errorbar(aes(ymin = Average - SE, ymax = Average +SE), width=0.2, position = position_dodge(0.95)) +labs(y=paste("Average ",Mood," Score",sep=""), x = "Gender", fill = gene_combination) + theme_classic() + 
    scale_fill_manual(values=c("#FFFFFF", "#808080"))+coord_cartesian(ylim= c(20,60))
  
  
  
  
  summary = summarize(group_by(data_subset, !!sym(gene1),!!sym(gene2)), Average = mean(!!sym(Mood)), SE = (sd(!!sym(Mood))/sqrt(length(!!sym(Mood)))))
  ...
  summary_gendered_gene = summarize(group_by(data_subset, !!sym(gene1),!!sym(gene2), Gender), Average = mean(!!sym(Mood)), SE = (sd(!!sym(Mood))/sqrt(length(!!sym(Mood)))))
  plot_4 = ggplot(summary_gendered_gene, aes(!!sym(gene1), Average, fill = !!sym(gene2)))+
  geom_col(position = position_dodge(0.95, preserve = 'single')) + geom_bar(stat = "identity", color = 'black',
                                          width=0.9, position=position_dodge(0.95, preserve = "single"))+
  geom_errorbar(aes(ymin = Average - SE, ymax = Average +SE), width=0.2, position = position_dodge(0.95, preserve = 'single')) +labs(y=paste("Average ",Mood," Score",sep=""), x = gene1) + theme_classic() + 
    scale_fill_manual(values=c("#FFFFFF", "#808080", '#D3D3D3'))+ facet_wrap(~ Gender)+coord_cartesian(ylim= c(20,60))
